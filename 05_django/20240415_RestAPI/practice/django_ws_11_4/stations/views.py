...
from .serializers import LocationSerializer, StationListSerializer, StationSerializer, CarSerializer, CarFixSerializer
...
def car_detail(request, car_pk):
    ...
    if request.method == 'GET':
        ...
        serializer = CarFixSerializer(car, data=request.data)
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            serializer = CarSerializer(car)
            return Response(serializer.data)