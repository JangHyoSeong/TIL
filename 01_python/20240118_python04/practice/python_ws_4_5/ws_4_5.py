    ...
    for user in user_data:
        ...
        if result == 'blocked':
            ...
            for i in range(0, len(result[1])):
                user[result[1][i]] = None
            
            user_list.append(user)
    print(f'잘못된 데이터로 구성된 유저의 수는 {wrong_data_count}입니다.')
    print(user_list)
...
def is_validation(user):
    ...
    if blood_type not in blood_types:
        what_is_wrong.append('blood group')

    if company in black_list:
        return 'blocked'
    
    if '@' not in mail:
        what_is_wrong.append('email')

    if len(name) < 2 or len(name) > 30:
        what_is_wrong.append('name')

    if len(user['website']) < 1:
        what_is_wrong.append('website')

    if len(what_is_wrong) == 0:
        return True, None
    
    return False, what_is_wrong
